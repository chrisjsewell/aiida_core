...
class TestNodeHashing:
    ...
    def create_simple_node(self, a, b=0, c=0):
        n = orm.Data()
        n.set_attribute('a', a)
        n.set_attribute('b', b)
        n.set_attribute('c', c)
    ...
    def create_folderdata_with_empty_file(self):
        node = orm.FolderData()
        with tempfile.NamedTemporaryFile() as handle:
            node.put_object_from_filelike(handle, 'path/name')
        return node

    def create_folderdata_with_empty_folder(self):
...
class TestTypes:
    ...
    def test_uuid_type(self):
        ...
        for uuid, data in results:
            assert isinstance(uuid, str) is True
            assert isinstance(data.uuid, str) is True
...
class TestQueryWithAiidaObjects:
    ...
    def test_with_subclasses(self, aiida_localhost):
        ...
        a1 = orm.CalcJobNode(computer=aiida_localhost)
        ...
        a6 = orm.CalcJobNode(computer=aiida_localhost)
...
class TestNodeBasic:
    ...
    def test_uuid_uniquess(self):
        ...
        with pytest.raises((DjIntegrityError, SqlaIntegrityError)):
            b.store()
    ...
    def test_code_loading_from_string(self, aiida_localhost):
        """
        ...
        code1 = orm.Code()
        code1.set_remote_computer_exec((aiida_localhost, '/bin/true'))
        code1.label = 'test_code1'
        code1.store()

        code2 = orm.Code()
        code2.set_remote_computer_exec((aiida_localhost, '/bin/true'))
        ...
        q_code_2 = orm.Code.get_from_string(code2.label + '@' + aiida_localhost.label)
    ...
    def test_code_loading_using_get(self, aiida_localhost):
        """
        ...
        q_code_2 = orm.Code.get(label=code2.label, machinename=aiida_localhost.label)
        assert q_code_2.id == code2.id
        assert q_code_2.label == code2.label
        assert q_code_2.get_remote_exec_path() == code2.get_remote_exec_path()
        ...
        code4 = orm.Code()
        code4.set_remote_computer_exec((aiida_localhost, '/bin/true'))
    ...
    def test_code_description(self, aiida_localhost):
        """
    ...
    def test_list_for_plugin(self, aiida_localhost):
        """
...
class TestSubNodesAndLinks:
    ...
    def test_valid_links(self, aiida_localhost):
        from aiida.plugins import DataFactory
        ...
        calc = orm.CalcJobNode(computer=aiida_localhost)
        calc.set_option('resources', {'num_machines': 1, 'num_mpiprocs_per_machine': 1})

        calc2 = orm.CalcJobNode(computer=aiida_localhost)
        ...
        calc_a = orm.CalcJobNode(computer=aiida_localhost)
        calc_a.set_option('resources', {'num_machines': 1, 'num_mpiprocs_per_machine': 1})
        calc_a.store()
        calc_b = orm.CalcJobNode(computer=aiida_localhost)
    ...
    def test_check_single_calc_source(self, aiida_localhost):
        """
